...
class _HomeState extends State<Home> {
  ...
  getCartItemLength() async{
    int price;
    FirebaseFirestore.instance.collection('users').doc(FirebaseAuth.instance.currentUser!.email).collection('cartitems').get().then((snap) => {
      if(snap.docs.isNotEmpty){
        cartController.cartLength.value = snap.docs.length,
        for(var x=0; x < snap.docs.length; x++){
          cartController.addPrice(snap.docs[x]['price'.toString()],snap.docs[x]['quantity']),
        }
      }
      else{
  ...
  showDialog(String msg){
    Get.defaultDialog(
      ...
      confirm: ElevatedButton(onPressed: (){
       controller.isDeviceConnected.value == false ? Container() : Navigator.of(context).pop();
      }, child: Text("OK"))
    );
  }
