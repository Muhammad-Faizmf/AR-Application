...
class CartItemSample extends StatelessWidget {
  ...
  increaseCartItemQuantity(String id, image, String name, String price, int qty) async {
   ...
   FirebaseFirestore.instance.collection('users').doc(FirebaseAuth.instance.currentUser!.email).collection('cartitems').doc(id).update({
      'image' : image, 'name' : name, 'price' : price, 'quantity' : itemcontroller.itemquanity.value
    });
   ...
   decreaseCartItemQuantity(String id, image, String name, String price, int qty) async {
   itemcontroller.itemquanity.value = qty;
   itemcontroller.itemquanity.value--;
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: FirebaseFirestore.instance.collection('users').doc(FirebaseAuth.instance.currentUser!.email).collection('cartitems').snapshots(),
                    ...
                    children: [
                      InkWell(
                        onTap: () async {
                          ...
                          cartcontroller.cartLength.value = cartcontroller.cartLength.value - 1;

                          //  deleting cart item price
                          cartcontroller.deductPrice(storedoc[x]['price'.toString()],
                          storedoc[x]['quantity']
                          );
                        },
                        child: const Icon(
                      ...
                      Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                        ...
                        children: [
                          InkWell(
                            ...
                            onTap: () {
                              decreaseCartItemQuantity(
                                storedoc[x]['id'],
                                storedoc[x]['image'], storedoc[x]['name'],storedoc[x]['price'], storedoc[x]['quantity']
                              );
                            },
                            child: Container(
                          ...
                          Container(
                            ...
                            child: storedoc[x]['quantity'] <= 9 ? Text("0${storedoc[x]['quantity'].toString()}") : Text(storedoc[x]['quantity'].toString()),
                          ),
                          InkWell(
                            borderRadius: BorderRadius.circular(20.0),
                            onTap: () async {
                              cartcontroller.increaseQuantity(storedoc[x]['price'.toString()], 
                              storedoc[x]['quantity']
                              );
